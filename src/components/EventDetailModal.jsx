  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto">
      <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        <div className="fixed inset-0 transition-opacity bg-black/70 backdrop-blur-sm" onClick={closeNEODetails} />
              ...
              <div className="flex items-center space-x-2">
                {neo.nasa_jpl_url && (
                  <a
                    ...
                    className="p-2 bg-cosmic-blue/20 hover:bg-cosmic-blue/30 rounded-lg transition-colors text-cosmic-gold hover:text-white"
          ...
          <div className="px-6 py-6 max-h-96 overflow-y-auto">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="space-y-6">
                <div>
                  <h3 className="text-lg font-semibold text-white mb-4">Close Approach Details</h3>
                  <div className="space-y-4">
                    <InfoItem
                      icon={Calendar}
                      label="Close Approach Date"
                      value={formatDateTime(closeApproach.close_approach_date_full || closeApproach.close_approach_date)}
                    />
                    ...
                    <InfoItem
                      ...
                      value={
                        <div className="space-y-1">
                          <div>{formatDistance(closeApproach.miss_distance?.kilometers || 0)}</div>
                          ...
                          {closeApproach.miss_distance?.lunar && (
                            <div className="text-sm text-gray-400">
                              {parseFloat(closeApproach.miss_distance.lunar).toFixed(1)} Lunar Distances
                            </div>
                          )}
                        </div>
                      }
                    ...
                    <InfoItem
                      ...
                      value={
                        <div className="space-y-1">
                          <div>{formatVelocity(closeApproach.relative_velocity?.kilometers_per_hour || 0)}</div>
              ...
              <div className="space-y-6">
                <div>
                  <h3 className="text-lg font-semibold text-white mb-4">Physical Characteristics</h3>
                  <div className="space-y-4">
                    <InfoItem
                      icon={Ruler}
                      ...
                      value={
                        <div className="space-y-1">
                          <div>
                            {formatDiameter(diameter.estimated_diameter_min || 0)} - {formatDiameter(diameter.estimated_diameter_max || 0)}
                          </div>
                          <div className="text-sm text-gray-400">
                            Average: {formatDiameter(((diameter.estimated_diameter_min || 0) + (diameter.estimated_diameter_max || 0)) / 2)}
                          </div>
                        </div>
                      }
                    />
                ...
                {Object.keys(neo.estimated_diameter || {}).length > 1 && (
                  <div>
                    <h3 className="text-lg font-semibold text-white mb-4">Size Estimates</h3>
                    <div className="space-y-3">
                      {Object.entries(neo.estimated_diameter || {}).map(([unit, data]) => (
                        <div key={unit} className="bg-white/5 rounded-lg p-3">
                          ...
                          <div className="flex justify-between text-sm">
                            ...
                            <span className="text-white">
                              {parseFloat(data.estimated_diameter_min || 0).toFixed(
                                unit === 'kilometers' ? 3 : unit === 'meters' ? 1 : 2
                              )} {unit === 'kilometers' ? 'km' : unit === 'meters' ? 'm' : unit === 'miles' ? 'mi' : 'ft'}
                            </span>
                          </div>
                          <div className="flex justify-between text-sm">
                            <span className="text-gray-400">Max:</span>
                            <span className="text-white">
                              {parseFloat(data.estimated_diameter_max || 0).toFixed(
                                unit === 'kilometers' ? 3 : unit === 'meters' ? 1 : 2
                              )} {unit === 'kilometers' ? 'km' : unit === 'meters' ? 'm' : unit === 'miles' ? 'mi' : 'ft'}
                            </span>
                          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="text-center">
                  ...
                  <p className={`font-medium ${
                    neo.is_potentially_hazardous_asteroid ? 'text-red-300' : 'text-green-300'
                  }`}>
                    {neo.is_potentially_hazardous_asteroid ? 'Potentially Hazardous' : 'Safe'}
                  </p>
                </div>
                ...
                <div className="text-center">
                  ...
                  <p className="font-medium text-blue-300">
                    {closeApproach.miss_distance?.lunar && parseFloat(closeApproach.miss_distance.lunar) < 10
                      ? 'Very Close' 
                      : closeApproach.miss_distance?.lunar && parseFloat(closeApproach.miss_distance.lunar) < 50
                      ? 'Close'
                      : 'Distant'}
                  </p>
                </div>
                ...
                <div className="text-center">
                  ...
                  <p className="font-medium text-purple-300">
                    {((diameter.estimated_diameter_min || 0) + (diameter.estimated_diameter_max || 0)) / 2 >= 1
                      ? 'Large' 
                      : ((diameter.estimated_diameter_min || 0) + (diameter.estimated_diameter_max || 0)) / 2 >= 0.1
                      ? 'Medium'
                      : 'Small'}
                  </p>
                </div>
