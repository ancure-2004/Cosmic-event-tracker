...
import { Calendar, BarChart3, RefreshCw, AlertCircle, Telescope } from 'lucide-react'
...
const HomePage = () => {
  ...
  const neosByDate = useMemo(() => {
    ...
    filteredNEOs.forEach(neo => {
      const date = neo.close_approach_data?.[0]?.close_approach_date || neo.approach_date
      if (!grouped[date]) {
        grouped[date] = []
      }
  ...
  const stats = useMemo(() => {
    ...
    const hazardous = filteredNEOs.filter(neo => neo.is_potentially_hazardous_asteroid).length
    const safe = total - hazardous
    
    const avgDiameter = filteredNEOs.reduce((sum, neo) => {
      const diameter = neo.estimated_diameter?.kilometers || {}
      return sum + ((diameter.estimated_diameter_min || 0) + (diameter.estimated_diameter_max || 0)) / 2
    }, 0) / (total || 1)
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            <div className="card p-6 text-center">
              ...
              <p className="text-gray-300 text-sm">
                Access up-to-date information about Near-Earth Objects from NASA's database
              </p>
            </div>
            
            <div className="card p-6 text-center">
              <BarChart3 className="h-8 w-8 text-cosmic-gold mx-auto mb-3" />
              <h3 className="text-lg font-semibold text-white mb-2">Compare & Analyze</h3>
              <p className="text-gray-300 text-sm">
                Select multiple NEOs to compare their characteristics and trajectories
              </p>
            </div>
            ...
            <div className="card p-6 text-center">
              ...
              <p className="text-gray-300 text-sm">
                Identify potentially hazardous asteroids and monitor their approach
              </p>
            </div>
          </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-gray-300">
              Tracking cosmic events from {new Date(dateRange.start).toLocaleDateString()} 
          ...
          <div className="flex items-center space-x-4">
            ...
            <button
              ...
              className={`btn-primary ${selectedNEOs.length < 2 ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
          ...
          {showStats && (
            <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4 border border-white/20">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                ...
                <div>
                  <p className="text-2xl font-bold text-red-400">{stats.hazardous}</p>
                ...
                <div>
                  <p className="text-2xl font-bold text-blue-400">{stats.avgDiameter}</p>
        ...
        <div className="space-y-8">
          {Object.keys(neosByDate).length === 0 ? (
            <div className="text-center py-12">
              ...
              <h3 className="text-xl font-medium text-white mb-2">No NEOs found</h3>
